...
const LEFT_ITEMS: DockItem[] = [
  { id: "dashboard", label: "Dash", icon: LayoutDashboard, path: "/admin", end: true },
  { id: "users", label: "Usuários", icon: Users, path: "/admin/users" },
];

const RIGHT_ITEMS: DockItem[] = [
  { id: "tickets", label: "Suporte", icon: Ticket, path: "/admin/tickets" },
];

const MORE_ITEMS: DockItem[] = [
  { id: "subscriptions", label: "Assinaturas", icon: CreditCard, path: "/admin/subscriptions" },
  { id: "downgrades", label: "Downgrades", icon: ArrowDownCircle, path: "/admin/downgrade-requests" },
  { id: "financial", label: "Financeiro", icon: DollarSign, path: "/admin/financial" },
  { id: "pix", label: "PIX Pendentes", icon: QrCode, path: "/admin/pix-payments" },
  { id: "pixConfig", label: "Config. PIX", icon: QrCode, path: "/admin/pix-config" },
  { id: "families", label: "Famílias", icon: UserRound, path: "/admin/families" },
  { id: "analytics", label: "Analytics", icon: BarChart3, path: "/admin/analytics" },
  { id: "logs", label: "Logs", icon: FileText, path: "/admin/logs" },
  { id: "settings", label: "Configurações", icon: Shield, path: "/admin/settings" },
  { id: "llm-config", label: "Config. LLM", icon: Settings, path: "/admin/llm-config" },
];
            ...
            <div className="mt-1 border-t border-white/10 px-1 pt-2">
              <button
                ...
                className="flex h-14 w-full items-center gap-3 rounded-2xl px-3
                  text-white/85 transition-colors duration-200 hover:bg-white/5 hover:text-white"
              >
                <span className="relative flex h-9 w-9 shrink-0 items-center justify-center rounded-full bg-white/5">
                  <LogOut size={20} strokeWidth={1.8} className="text-white/85" />
                </span>
                <span className="flex-1 truncate text-left text-sm font-medium">
                  Voltar ao App
                </span>
          ...
          <div className="flex items-center gap-1">
            {LEFT_ITEMS.map((item) => (
              <DockButton
                key={item.id}
                item={item}
                active={isActivePath(item.path, item.end)}
